"""
===========================================================
 Experiment rotated MNIST: full
===========================================================

This script provides pipelines for the experiments with full rotated version
...
from gpflow.kernels import SquaredExponential, Convolutional, LinearCoregionalization
...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="./checkpoints/mnist_rot",
    ...
    parser.add_argument("--model", type=str,
                        choices=["SE", "Rinv", "ConvSE", "RinvConvp8andRinv", "RinvConvandRinv"],
                        required=True,
                        help="Model type")
...
def build_model(Xtrain, arg_dict):
    ...
    def sample_inducing_patches(Xtrain, arg_dict, kernel, r_uniform=0.5):
    ...
    if arg_dict["model"] == "SE":
        ...
        for i in range(arg_dict["num_classes"]):
            ...
            base_kernel.variance = gpflow.Parameter(base_kernel.variance.numpy(), transform=var_constrained())
        ...
        W = to_default_float(W)
        kernel = LinearCoregionalization(kernel_list, W=W)
        # Set mixing matrix W to non-trainable. Results in simply summing the right latent GPs
        gpflow.set_trainable(kernel.W, False)
        Z = SeparateIndependentInducingVariables(Z_list)
        model = initialise_SVGP_model(kernel, likelihood, Z, arg_dict)
    ...
    elif arg_dict["model"] == "RinvConvp8andRinv":
        latent_per_class = 2
        arg_dict["num_latent"] = latent_per_class * arg_dict["num_classes"]
        ...
        Z_list = list()
        for i in range(arg_dict["num_classes"]):
            # Rotation equivariant convolutional component
            var = var_init_patch
            l_scales = lscale_init_patch
            base_kernel = SquaredExponential(variance=var, lengthscales=l_scales)
            base_kernel.variance = gpflow.Parameter(base_kernel.variance.numpy(), transform=var_base_constrained())
            ...
            kernel = RotationInvarConvolutional(
                ...
                normalisation_factor_scale=8 # 2 * group order
            )
            kernel.weights = gpflow.Parameter(
                kernel.weights.numpy(),
                transform=max_abs_w_patch()
    ...
    elif arg_dict["model"] == "RinvConvandRinv":
        latent_per_class = 2
        arg_dict["num_latent"] = latent_per_class * arg_dict["num_classes"]
